	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try
		{
			...
			else
			{
				...
				long destination = Long.parseLong(request.getParameter("destination"));
				float amount = Float.parseFloat(request.getParameter("amount_a2a"));
				String transactionPassword = request.getParameter("trans_pass_a2a");
				...
				else
				{
					...
					query = "select balance from accountDetails where accountDetails.accountNumber="+destination;
					...
					if( paymentResult.next())
					{
						...
						transactionID = getCount("transactionID",paymentStatement,pw);
						//debit from source account id balance is sufficient
						isDebited = DebitFromAccount(paymentStatement,source,deductableAmount,amount+" Rs. Transfered to Account Number "+destination,transactionID );
						...
						if(isDebited)
						{
							...
							CreditFromAccount(paymentStatement,destination,amount,amount+" Rs. Transferred by Account number "+source,transactionID  );
						
							//make entry in transaction Table
							makeTransactionEntries(transactionID,date,time,paymentStatement,pw);
							
							//remittance(accountIssueFrom bigint,accountIssueTo bigint,amount float,dateIssue date,number int,personIssueTo varchar(20),status varchar(20)
							//transferred
							query = "insert into remittance values("+source+","+destination+","+amount+",'"+date+"',"+transactionID+")";
							ExecuteQuery(query,paymentStatement,false);
							
							responsePacket = "{'success':true,'data':'"+transactionID+"'}";
