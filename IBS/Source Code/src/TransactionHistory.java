 ...
 *
 */
 public class TransactionHistory extends DBConnect implements javax.servlet.Servlet {
   static final long serialVersionUID = 1L;
	 ...
	 */
	/*Parameters:  "acnum_trans_history"  : Account Number
                    "startdt_trans" : Start Date
                    "enddt_trans" : End Date
		You need to search for all the transactions done from the table remittance for the given date range. For making a query of date range, you can do select * from remittance where date>=start_date and date<=enddate. these type of queries are also valid
		Reponse: JSON STring
		"{'success':true/false,'size': <no. of records>,'data':[{'id':1,'to':'<issued to a/c num>','tid':'<transaction id>','amount':'<amount>','date':'<date of transfer>'},{....2nd record and so on}]}"
	*/
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try
        {
        	...
        	{
        		 //pw.println("check3");
        		accountNum= Long.parseLong((request.getParameter("acnum_trans_history")));
        		startDate = request.getParameter("startdt_trans");
        		endDate = request.getParameter("enddt_trans");
        		 //pw.println("check4");
        		//create table remittance(accountIssueFrom bigint,accountIssueTo bigint,amount float,dateIssue date,transactionID int)
        		
        		query = "select * from remittance where accountIssueFrom ="+accountNum+" and dateIssue <='"+endDate+"' and dateIssue>='"+startDate+"'";
        		 //pw.println(query);
        		...
        		while(rset.next()) // drafts exists
        		{
        			...
        			tempPacket += "{'id':"+id+",'to':'"+acIssuedTo+"','tid':'"+transactionID+"','amount':'"+amount+"','date':'"+dateOfTransfer+"'}";
        			
        			 //pw.println(tempPacket);
        		}
        		if(id==0)	// no such transaction on this accountNum
        			responsePacket = "{success:false}";
        		else	//  make response packet
        			{responsePacket = "{'success':true,'size':"+id+",'data':["+tempPacket+"]}";
        			 //pw.println("check7");
