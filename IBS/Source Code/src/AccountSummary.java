 ...
 */
 public class AccountSummary extends DBConnect implements javax.servlet.Servlet {
   static final long serialVersionUID = 1L;
    ...
    //Json String
    //                "{'success': true,'size': <no. of records sending>,'data' : [{'id':1,'acnum':'<A/c num1>','actype':'<type of a/c1>' ,'holder':'<any 1 holder of a/c>','balance':'balance1'},
    //                { <similarly for 2nd record and so on>}]}"
   
    public String createJSONString( int dataCount, long accountNumber, String accountType, String holder, float balance )
    {
        String Packet = "{'id':"+dataCount+",'acnum':'"+accountNumber+"','actype':'"+accountType+"','holder':'"+holder+"','balance':'"+balance+"'}";
        return Packet;
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	try
        {
            ...
            else
            {
                ...
                //variables to store value retrieved from database for a particular table
                ...
                query = "select accountNumber from accountMapping where accountMapping.userID="+UserID;
                //pw.println(query);
                ResultSet accountResult = ExecuteQuery(query,summaryStatement1,true);
               
                while(accountResult.next())
                {
                    //account number mapped on this userID
                    accountNumber = accountResult.getLong("accountNumber");
                    query = "select accountType,holder1,balance from accountDetails where accountDetails.accountNumber="+accountNumber;
                    //pw.println(query);
                    Statement summaryStatement2 = CreateQueryStatement(summaryConnection);
                    ResultSet accountDetailsResult = ExecuteQuery(query,summaryStatement2,true);
                   
                    if(accountDetailsResult.next())
                    {
                        accountType = accountDetailsResult.getString("accountType");
                        holder = accountDetailsResult.getString("holder1");
                        balance = accountDetailsResult.getFloat("balance");
                   
                        if( isFirst )
                        {
                        	responsePacket = responsePacket + createJSONString( count,accountNumber,accountType,holder,balance );
                        	isFirst = false;
                        }
                        else
                        {
