...
/**
 * Servlet implementation class for Servlet: DraftHistory
 *
 */
 public class DraftHistory extends DBConnect implements javax.servlet.Servlet {
   static final long serialVersionUID = 1L;
	...
	//json response : "{'success': true/false,'size':<no . of draft records>,'data':[{'id':1,'num':'<draft number>','favour':'<in favour of>','amount':'<amount>','status':'<status>'},{.... 2nd record .... and so on}]}"
...
//A ResponsePacket to store the return values
        ...
        try
        {
        	...
        	{
        		accountNum= Long.parseLong((request.getParameter("acnum_history")));
        		...
        		while(rset.next()) // drafts exists
        		{
        			...
        			tempPacket += "{'id':"+id+",'num':'"+draftNum+"','favour':'"+favour+"','amount':'"+amt+"','status':'"+status+"'}";
