...
public class Payment extends DBConnect implements javax.servlet.Servlet{
	...
	// in derived class it can be set and there it may be used as a normal
	// statement
	
	// debit a particular amount to accountNumber
	public boolean CreditFromAccount(Statement paymentStatement,
	        long accountNumber, float amount, String details, int transactionId)
	{
		try
		{
			// make entry in debit table
			query = "insert into credit values(" + accountNumber + "," + amount
			        + "," + transactionId + ",'" + details + "')";
			ExecuteQuery(query, paymentStatement, false);
			
			// update balance in accountDetails table
			query = "update accountDetails set accountDetails.balance=accountDetails.balance+"
			        + amount
			        + " where accountDetails.accountNumber="
			        + accountNumber;
			ExecuteQuery(query, paymentStatement, false);
	        ...
	        long accountNumber, float amount, String details, int transactionId)
	{
		...
		try
		{
			...
			{
				// balance of account number is greater than amount to be
				// credited
				
				// make entry in credit table
				query = "insert into debit values(" + accountNumber + ","
				        + amount + "," + transactionId + ",'" + details + "')";
				ExecuteQuery(query, paymentStatement, false);
	...
	public boolean makeTransactionEntries(int transactionID, String date,
	        String time, Statement paymentStatement, PrintWriter pw)
	{
		query = "insert into transaction values(" + transactionID + ",'" + time
		        + "','" + date + "')";
		ExecuteQuery(query, paymentStatement, false);
		return true;
	}
	
	public void updateCountByOne(String field, Statement paymentStatement,
	        PrintWriter pw)
	{
		query = "update count set count.value=count.value+1 where count.cnt='"
		        + field + "'";
		ExecuteQuery(query, paymentStatement, false);
	}
	
	public boolean VarifyTransactionPassword(Statement paymentStatement,
	        String enterdPassword, String UserID, boolean isPersonal,
	        PrintWriter pw)
	{
		...
		if(isPersonal)
			query = "select * from ploginInfo where ploginInfo.userID="
			        + UserID + " and ploginInfo.transactionPass='"
			        + enterdPassword + "'";
		else
			query = "select * from cloginInfo where cloginInfo.userID="
			        + UserID + " and cloginInfo.transactionPass='"
			        + enterdPassword + "'";
