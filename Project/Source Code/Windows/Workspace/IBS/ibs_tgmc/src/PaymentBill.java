...
/**
 * Servlet implementation class for Servlet: PaymentBill
 * 
 */
public class PaymentBill extends GeneratePDF implements javax.servlet.Servlet{
	        ...
	        HttpServletResponse response) throws ServletException, IOException
	{
		...
		try
		{
			...
			else
			{
				...
				String nick = request.getParameter("nick_present1");
				long billNumber = Long.parseLong(request
				        .getParameter("bill_num_present1"));
				float amount = Float.parseFloat(request
				        .getParameter("amount_present1"));
				long accountNumber = Long.parseLong(request
				        .getParameter("ac_num_present1"));
				String transactionPassword = request
				        .getParameter("pass_present1");
				...
				// 3.insert into the table bill info
				...
				{ // transaction password is correct
					...
					if(isDebited)
					{
						...
						// billInfo(accountNumber bigint,provider
						// varchar(80),billNumber bigint,amountPaid
						// float,dateTransaction date,timeTransaction
						// time,TransactionID int)
						query = "insert into billInfo values(" + accountNumber
						        + ",'" + provider + "'," + billNumber + ","
						        + amount + ",'" + date + "','" + time + "',"
						        + transactionID + ")";
						// pw.println(query);
						...
						// String path =
						// dir+"content/Draft"+accountNumber+".pdf"; //for linux
						// server
